...
async function readRaceCache(date: string): Promise<Race[] | null> {
...
async function saveRaceCache(date: string, races: Race[]) {
  const db = await openRaceDb();
...
function selectTopBetsByRace(bets: BetResult[], topN: number): BetResult[] {
  const groups = bets.reduce<Record<string, BetResult[]>>((acc, bet) => {
    if (!acc[bet.race]) acc[bet.race] = [];
    acc[bet.race].push(bet);
    return acc;
  }, {});

  return Object.values(groups).flatMap((raceBets) =>
    raceBets.sort((a, b) => b.ev - a.ev).slice(0, topN)
  );
}

function applyPopularSideFilter(bets: BetResult[]): BetResult[] {
  return bets.filter((b) => {
    return !(b.odds <= 8 && b.ev < 6);
  });
}

function summarizeBets(bets: BetResult[], stake: number) {
  const investment = bets.length * stake;

  const payout = bets.reduce(
    (sum, b) => sum + (b.hit ? ((b.payout ?? 0) / 100) * stake : 0),
    0
  );
      ...
      try {
        ...
        for (const date of dates) {
          ...
          const res = await fetch(
            "/api/boatrace?mode=backtest&startDate=" +
              date +
              "&endDate=" +
              date
          );
...
allBets.push({
  ...
  hit: prediction.bet === race.result,
    ...
    const evComparisons = [5.0, 5.5, 6.0, 6.5, 7.0].map((threshold) => {
      ...
      const comparisonBetsRaw = selectTopBetsByRace(filtered, 1);
      const comparisonBets = applyPopularSideFilter(comparisonBetsRaw);
      const summary = summarizeBets(comparisonBets, stake);
    ...
    const evBandComparisons = [
      { label: "EV 5.0 - 5.5", min: 5.0, max: 5.5 },
      { label: "EV 5.5 - 6.0", min: 5.5, max: 6.0 },
      { label: "EV 6.0 - 6.5", min: 6.0, max: 6.5 },
      { label: "EV 6.5 - 7.0", min: 6.5, max: 7.0 },
      { label: "EV 7.0以上", min: 7.0, max: Number.POSITIVE_INFINITY },
    ].map((band) => {
      const filtered = allBets.filter(
        (b) =>
          b.ev >= band.min &&
      ...
      const bandBetsRaw = selectTopBetsByRace(filtered, 1);
      const bandBets = applyPopularSideFilter(bandBetsRaw);
      const summary = summarizeBets(bandBets, stake);

      return {
        label: band.label,
        betCount: bandBets.length,
        hitCount: summary.hitCount,
        hitRate: summary.hitRate,
        investment: summary.investment,
        payout: summary.payout,
